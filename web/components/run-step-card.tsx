...
const RunStepCard = ({ step, index }: RunStepCardProps) => {
	...
	return (
		<Card className={cn('border-slate-200 dark:border-slate-800', isFailed && 'border-rose-200 bg-rose-50/40 dark:border-rose-700/60 dark:bg-rose-950/30')}>
			<CardHeader className='space-y-3'>
				<div className='flex flex-wrap items-start justify-between gap-3'>
					<div className='space-y-1'>
						...
						<p className='text-xs text-slate-500 dark:text-slate-400'>Retries: {step.retries}</p>
					</div>
			...
			<CardContent className='space-y-3 pt-4'>
				{!hasInput && !hasOutput && !hasError ? (
					<p className='text-sm text-slate-500 dark:text-slate-400'>No payloads recorded for this step.</p>
				) : (
					<Accordion type='multiple' className='w-full'>
							...
							<AccordionItem value={`input-${step.id}`}>
								...
								<AccordionContent>
									<ScrollArea className='max-h-56 w-full rounded-md border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950'>
										<pre className='p-3 text-xs text-slate-700 dark:text-slate-200'>{stringifyPayload(step.input)}</pre>
									</ScrollArea>
								</AccordionContent>
							</AccordionItem>
						)}
						{hasOutput && (
							<AccordionItem value={`output-${step.id}`}>
								...
								<AccordionContent>
									<ScrollArea className='max-h-56 w-full rounded-md border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950'>
										<pre className='p-3 text-xs text-slate-700 dark:text-slate-200'>{stringifyPayload(step.output)}</pre>
									</ScrollArea>
								</AccordionContent>
							</AccordionItem>
						)}
						{hasError && (
							<AccordionItem value={`error-${step.id}`}>
								...
								<AccordionContent>
									<div className='rounded-md border border-rose-200 bg-rose-50 p-3 text-xs text-rose-700 dark:border-rose-700/60 dark:bg-rose-950/40 dark:text-rose-200'>{step.error}</div>
								</AccordionContent>
							</AccordionItem>
						)}
					</Accordion>
