...
const stepStatuses: RunStepStatus[] = ['pending', 'running', 'success', 'failed'];
...
const computeDurationMs = (startedAt?: string, endedAt?: string, durationMs?: number): number | undefined => {
...
export const mapHiveRunToUIRun = (raw: Record<string, unknown>): Run => {
	...
	const durationMs = computeDurationMs(startedAt, endedAt, toNumberValue(raw.durationMs ?? raw.duration_ms));
	...
	return {
		...
		steps: rawSteps.map((step) => mapHiveRunStepToUIRunStep(step as Record<string, unknown>)),
	};
};
...
const mockHiveRuns: Record<string, unknown>[] = [
	{
		...
		steps: [
			...
			},
			{
				...
				output: {
					summary: 'Key changes include updated runtime hooks and improved trace metadata.',
				},
